...
# General {
    ...
    if [ -n $SSH_CLIENT ]; then
        export TMUX_POWERLINE_THEME="local-theme"
    else
        export TMUX_POWERLINE_THEME="remote-theme"
    fi
...
# mailcount.sh {
	...
	export TMUX_POWERLINE_SEG_MAILCOUNT_MAILDIR_INBOX="/home/felinae/.mail/inbox/new"
	
	## mbox
	# Path to the mbox to check.
	export TMUX_POWERLINE_SEG_MAILCOUNT_MBOX_INBOX="/var/spool/mail/felinae"
	
	## mailcheck
	# Optional path to mailcheckrc
	export TMUX_POWERLINE_SEG_MAILCOUNT_MAILCHECKRC="/home/felinae/.mailcheckrc"
# }
